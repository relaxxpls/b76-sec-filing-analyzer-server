...
# EBITDA == EBIT + Depreciation and Amortization costs
ebit = {}
ebitda = {}
...
for company in tqdm(os.listdir('json')):
    try:
        for file_name in glob('json/{}/10-K*'.format(company)):
            operating_expenses = {}
            with open(file_name, 'r', encoding='utf-8') as file:
                    data = json.load(file)

            # EBIT extractor
            # default_key_vals = ['StatementsOfIncome', 'StatementsOfComprehensiveIncome']
            ...
            # ROE extractor
            default_key_vals4 = ['StatementsOfIncome', 'StatementsOfComprehensiveIncome']
            for df in default_key_vals4:
                if df in data:
                    default_key4 = df
                    break
            default_key_net_inc = 'NetIncomeLoss'
            values = data[default_key4][default_key_net_inc]
            for value in values:
                item = parse_income_data(value)
                if not item: continue

                if company not in net_income: net_income[company] = {}
                net_income[company]['SEP'.join(item[0])] = item[1]
